...
void HandlerManager::addHandler(Psychopomp::AsyncService* service,
  ...
  auto requestHandler =
      std::make_shared<SyncedRequestHandler>(std::in_place, service, completionQueue);
...
void HandlerManager::process(void* handlerTag, bool ok) {
  RequestHandlerTag* requestHandlerTag = reinterpret_cast<RequestHandlerTag*>(handlerTag);
  std::shared_ptr<SyncedRequestHandler> requestHandler = getSyncedRequestHandler(requestHandlerTag->tag);
  if (requestHandler) {
...
std::shared_ptr<SyncedRequestHandler>
