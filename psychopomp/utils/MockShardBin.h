...
class MockShardBin {
  public:
  explicit MockShardBin(int64_t dropTime, int64_t addTime) : dropTime_(dropTime), addTime_(addTime) {

  }

  void addShard(uint32_t shardId) {
    shardSchedule[shardId] = std::make_pair(system_clock::now() + addTime_, system_clock::from_time_t(std::numeric_limits<time_t>::max()));
  }
  ...
  std::unordered_map
    <uint32_t, 
     std::pair<time_point<system_clock>,  
     time_point<system_clock>>> shardSchedule; 
};
