 ...
 public:
  ...
  void addMapping(std::pair<Domain, DomainId> parent,
    ...
    for (auto child : childrenDomainIds) {
      childToParentMap_[childDomain][child].emplace(parentDomain,
                                                    parentDomainId);
    }
  ...
  std::pair<Domain, std::vector<DomainId>> getChild(
      Domain domain, DomainId domainId,
      std::vector<std::shared_ptr<MovementMap>> movementMaps) const {
